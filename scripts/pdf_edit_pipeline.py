...
OCR_ZOOM = float(os.environ.get("PDFNEST_OCR_ZOOM", "2.0"))
OCR_LANG = os.environ.get("PDFNEST_OCR_LANG", "eng")
...
def sample_background_hex(page: fitz.Page, rect: fitz.Rect) -> str:
    """
    Approximate background color near the text box.
    For white pages this returns white.
    For off-white / colored pages it tries a small neighborhood.
    """
    ...
    for clip in candidate_rects:
        try:
            ...
            # Use the first pixel as a simple sample.
...
def ocr_words_for_page(page: fitz.Page, zoom: float = OCR_ZOOM, lang: str = OCR_LANG) -> List[Dict[str, Any]]:
        ...
        for page_idx, page_data in enumerate(pages):
            ...
            for element in elements:
                ...
                try:
                    ...
                    # Best effort: box rendering first, fallback to baseline rendering.
                    try:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="PDF edit pipeline for native + OCR-based text editing")
    ...
    compile_parser.add_argument("original_pdf")
    compile_parser.add_argument("output_pdf")
    compile_parser.add_argument("pages_json")
    compile_parser.add_argument("--password", default="")

    args = parser.parse_args()

    try:
        if args.command == "extract":
            result = extract_document(args.input_pdf, args.password or None)
            print(json.dumps(result, ensure_ascii=False))
        elif args.command == "compile":
            compile_document(args.original_pdf, args.output_pdf, args.pages_json)
        return 0
    except Exception as e:
